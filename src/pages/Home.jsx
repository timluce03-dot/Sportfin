  ...
  return (
    <Link to={`/articles/${a.id}`} className="card card-hover group flex flex-col">
      ...
      <div className="p-4 flex flex-col flex-1">
        ...
        <h3 className="text-[13px] font-bold leading-snug mb-2 flex-1"
          style={{ color: 'var(--sf-text)', fontFamily: 'var(--sf-font-heading)' }}>{a.title}</h3>
        <p className="text-[11.5px] leading-relaxed line-clamp-2 mb-3" style={{ color: 'var(--sf-muted)' }}>{a.excerpt}</p>
  ...
  return (
    <div className="p-5 rounded-2xl border flex flex-col gap-4" style={{ background: 'var(--sf-surface)', borderColor: 'var(--sf-border)', boxShadow: 'var(--sf-shadow-xs)' }}>
      ...
      <div className="flex items-center gap-3 pt-2 border-t" style={{ borderColor: 'var(--sf-border)' }}>
        {r.avatar
          ? <img src={r.avatar} alt={name} className="w-11 h-11 rounded-full object-cover flex-shrink-0" style={{ border: '2px solid var(--sf-border)' }} />
          : <div className="w-11 h-11 rounded-full flex items-center justify-center text-white text-[13px] font-bold flex-shrink-0"
              style={{ background: 'var(--sf-primary)' }}>{initials}</div>
        }
        <div>
    ...
    <div className="flex items-center justify-center gap-1.5 mt-8">
      {Array.from({ length: pages }, (_, i) => i + 1).map(p => (
        <button key={p} onClick={() => onPage(p)}
          className={`w-8 h-8 rounded-lg text-[13px] font-semibold transition-colors ${p === page
            ? 'text-white' : 'text-gray-500 hover:bg-gray-100'}`}
          style={p === page ? { background: 'var(--sf-primary)' } : {}}>
          {p}
        </button>
      ))}
    </div>
...
function FeatureCarousel({ features, featureIcons }) {
  ...
  return (
    <section>
      {/* Bande titre */}
      <div style={{ background: 'var(--sf-surface)', borderBottom: '1px solid var(--sf-border)' }}>
        <div className="max-w-[1360px] mx-auto px-6 lg:px-10 py-3">
          <p className="text-[9.5px] font-bold tracking-[.2em] uppercase" style={{ color: 'var(--sf-muted)' }}>
...
export default function Home() {
  ...
  useEffect(() => {
    getArticles({}).then(({ data, error }) => {
      setAllArticles(data); setArticlesErr(error); setLoadingArticles(false)
    })
    getReviews({ limit: 8 }).then(({ data }) => {
      setReviews(data); setLoadingReviews(false)
    })
  }, [])
      ...
      <div className="grid grid-cols-1 xl:grid-cols-[1fr_300px] gap-12">
        ...
        <div>
          ...
          {loadingArticles ? (
            ...
            <>
              ...
              <Pagination page={page} total={allArticles.length} perPage={PAGE_SIZE} onPage={p => { setPage(p); window.scrollTo({ top: 0, behavior: 'smooth' }) }} />
            </>
          )}
        </div>
        ...
        {theme.sections?.showReviews !== false && (
          <aside>
            ...
            {loadingReviews ? (
              <div className="flex flex-col gap-3">{[1,2,3].map(i => <div key={i} className="h-28 skeleton rounded-xl" />)}</div>
            ) : reviews.length === 0 ? (
